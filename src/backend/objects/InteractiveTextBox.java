...
public class InteractiveTextBox extends TextObject implements ListenerObject {
    ...
    /**
     * Draws an orange rectangle representing the selection field of the object as
     * well as a text cursor.
     */
    ...
    */
    private void removeLine() {
        ycenter -= (textSize + LINE_SPACING / 4f) / 2f;
        if (pixelHeight > textSize + LINE_SPACING / 4f)
            pixelHeight -= textSize + LINE_SPACING / 4f;
    }
    ...
    private void setCurrentLine(String newCurrentLine) {
        ...
        str = "";
        for (i = 0; i < charactersAl.size(); i++)
            str += charactersAl.get(i);
    }
    ...
    private boolean inNumbers() {
        ...
        if (Character.isAlphabetic(firstNonWhitespaceCharacter) && Character.isLowerCase(firstNonWhitespaceCharacter)) {
            if (getIndentationLevel() % 2 == 1)
                return true;
        }
        if (Character.isDigit(firstNonWhitespaceCharacter)) {
            if (getIndentationLevel() % 2 == 0)
                return true;
        }
        return false;
    }
    ...
    private int getIndentationLevel() {
        ...
        if (indentationChars % INDENTATION_SIZE == 0)
            return indentationChars / INDENTATION_SIZE;
        else
            return 0;
    }
    ...
    private int scan(Directions d) {
        ...
        while (index >= 1 && index < strCharacters.length && !(strCharacters[index] == PConstants.RETURN)
                && !(strCharacters[index] == PConstants.ENTER)) {
            if (d == Directions.LEFT)
                index--;
            else if (d == Directions.RIGHT)
                index++;
    ...
    protected void addCharacter(char c) {
        ...
        if (charactersSinceNewLine % charactersPerLine == 0)
            addLine();
        cursorIndex++;
        ...
        if (c == PConstants.ENTER || c == PConstants.RETURN) {
            ...
            if (inBullets || inNumbers) {
                if (inBullets) {
                    ...
                    String nextNumber = "a";
                    if (lastNumber == 'z')
                        nextNumber = Character.toString('a');
                    else {
                    ...
                    for (int i = 0; i < nextNumberChars.length; i++)
                        addCharacter(nextNumberChars[i]);
                    addCharacter('.');
                    addCharacter(' ');
                }
    ...
    protected void removeCharacter() {
        ...
        if (cursorIndex > 0) {
            ...
            if (lineCursorIndex > 0) {
                for (int i = 0; i < BULLETS.length; i++)
                    if (BULLETS[i] == lineCharacters[lineCursorIndex - 1])
                        cursorCharIsBullet = true;
            }
     ...
     *                keys are the same)
     * @param keyCode Represents the value of they key currently pressed (capital
     *                and lower case letters are the same, all function keys are
     *                different)
     */
